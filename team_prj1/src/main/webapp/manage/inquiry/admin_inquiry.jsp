...
<jsp:useBean id="sVO" class="manage.inquiry.InquirySearchVO"
	scope="page" />
...
function setupResetButton() {
    ...
    function openPopup(inquiryId) {
        ...
        var width = 700;
        var height = 700;
        window.open("admin_inquiry_popup.jsp?inquiryId=" + inquiryId, "admin_inquiry_popup", 
...
%>
    ...
    <div id="main-content">
        ...
        <!-- 검색 필터 폼 -->
        <div class="form">
            <form method="GET" action="admin_inquiry.jsp" name="searchFrm" id="searchFrm" >
                ...
                <div class="form-group">
                    ...
                    <select id="filter" name="filter" class="form-select">
                        <option value="전체" <c:if test="${sVO.filter eq '전체'}">selected</c:if>>전체</option>
                        <option value="상품" <c:if test="${sVO.filter eq '상품'}">selected</c:if>>상품문의</option>
                        <option value="취소" <c:if test="${sVO.filter eq '취소'}">selected</c:if>>구매취소</option>
                        <option value="배송" <c:if test="${sVO.filter eq '배송'}">selected</c:if>>배송</option>
                    </select>
                </div>
            ...
            <div class="form">
                ...
                <table class="table table-striped table-hover">
                 <c:if test="${empty listBoard }">
                  <tr>
                      <td colspan="6" style="text-align: center;" > <strong>해당 조건에 맞는 검색 결과가 없습니다 </strong></td>
                  </tr>
               </c:if>      
